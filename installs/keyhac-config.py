...
##
## Windows の操作を emacs のキーバインドで行うための設定（Keyhac版）ver.20170306_01
##
...
def configure(keymap):
    ...
    # emacs日本語入力モード中のマウスクリックで、emacs日本語入力モードを OFF にする設定を有効にするかの
    # 指定をする（True: 有効、False: 無効）
    # （本設定で利用している mouseup フックが他のソフトに悪影響を与えているとの情報があることの暫定的な
    # 　対策。対策する場合は False に設定する）
    ei_mouse_setting = False

...
( avprobe -show_streams -show_format_entry duration "$video" 2>/dev/null | sed 's/^/Duration: /' ) \
    || echo "警告: 時間の抽出に失敗"
...
run avconv -i "$video" -loglevel error -threads ${threads} \
    -bufsize 40M -probesize 300M \
    -acodec libfdk_aac \
    -vcodec libx264 \
    -preset:v slow -tune:v animation -filter:v 'yadif=2' -f mp4 \
    "$mp4"
